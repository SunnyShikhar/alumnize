...
def beginTransform(almabase, oldFile):
                ...
                for i in employ_title_lst:
                    ...
                    if len(row[i[0]].strip()) != 0:
                        ...
                        clean_df.loc[count,["Duration",
                                            "Start.Date","Start.Month","Start.Year","End.Date.pres","End.Month","End.Year","Job.Duration"]] = a
    ...
    response['Content-Disposition'] = 'attachment; filename="masterFile-output.csv"'
    return response